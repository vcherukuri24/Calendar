...
- (UIViewController *) controllerAtIndex:(NSInteger) index {
     ...
     */
    self.calendar = [calendars objectForKey:[NSString stringWithFormat:@"%li",(long)index]];
    ...
    if (![controllers objectForKey:[NSString stringWithFormat:@"%li",(long)index]]) {
        ...
        self.calendar = [[UICollectionView alloc]initWithFrame:rect collectionViewLayout:flowLayout];
        self.calendar.dataSource = self;
        self.calendar.delegate = self;
        self.calendar.scrollEnabled = NO;
        [self.calendar registerClass:[SACalendarCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
        [self.calendar setBackgroundColor:calendarBackgroundColor];
        self.calendar.tag = index;
        ...
        [contr.view addSubview:self.calendar];
        
        [calendars setObject:self.calendar forKey:[NSString stringWithFormat:@"%li",(long)index]];
...
{
     monthToLoad = [self monthToLoad:(int)collectionView.tag];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth) {
        ...
        NSMutableArray *defaultsArray =[defaults objectForKey:@"FreJunTaskIndicatorArray"];
        
       
        for(int i = 0; i< [defaultsArray count];i++)
        {
            
            if((int)indexPath.row - firstDay + 1 == [[gregorian components:(NSCalendarUnitDay) fromDate:[defaultsArray objectAtIndex:i]] day] )
            {
                
                if([[gregorian components:(NSCalendarUnitMonth) fromDate:[defaultsArray objectAtIndex:i]] month]  == monthToLoad )
